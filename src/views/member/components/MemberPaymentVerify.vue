  ...
  <div class="app-container">
    <el-row>
      ...
      <el-col :span="24" align="center" style="margin-top:8px;margin-bottom:16px;">
        <span style="font-size: 16px;">{{ 'ส่งหลักฐานการชำระเงิน' }}</span>
      </el-col>
      <el-col :span="24" align="center" style="margin-top:8px;margin-bottom:16px;">
        <span style="font-size: 16px;">{{ orgData.TempHeader }}</span>
      </el-col>
      <el-col :span="24" align="center" style="margin-bottom:16px;">
...
import { getURL, getUserDetailData, getOrgType, getOrgId } from '@/utils/auth'
...
export default {
  ...
  created() {
    ...
    this.uploadUrl = getURL() + '/api/v1/mobile/slip'
    this.data = { member_id: this.profile.M01, area: this.profile.M02, bookList_id: this.bookList.B01, sendDate: dayjs(new Date()).format('YYYY-MM-DD HH:mm:ss') }
    console.log(this.data)
    this.uploadHeaders = { OrgId: getOrgId() }
  ...
  methods: {
    ...
    paymentClickLink(bookList) {
      this.$router.push({ path: '/member/payment_verify/' + bookList.B01 })
    },
    getDataUri(url) {
    ...
    createReference2(orgId, billId, memberId) {
      if (getOrgType() === 'prapa_all') {
        ...
        return this.formatOrg(orgId) + billId + memberId
      }
    },
    formatOrg(org) {
      if (getOrgType() === 'prapa_all') {
